...
fn main_loop(text_cmd_rx: Receiver<String>, rt: &tokio::runtime::Runtime) -> Result<(), ()> {
    ...
    'wake_word: loop {
        ...
        if audio_chunk_count % AUDIO_LOG_INTERVAL == 0 {
            debug!("[Audio] chunk #{} received ({} samples), vad_state={:?}",
                audio_chunk_count, frame_buffer.len(), vad_state);
        }
...
fn execute_command(text: &str, rt: &tokio::runtime::Runtime) -> bool {
    ...
    if let Some((cmd_path, cmd_config)) = cmd_result {
        ...
        if cmd_config.cmd_type == "script_ref" {
            ...
            if let Some(script) = scripts::load_script(&cmd_config.id) {
                ...
                match scripts::execute_script(&script) {
                    Ok(_) => {
                        ipc::send(IpcEvent::CommandExecuted { id: script.id.clone(), success: true });
                        ipc::send(IpcEvent::Idle);
                        return false;
                    }
                    Err(e) => {
                        ...
                        voices::play_error();
                        ipc::send(IpcEvent::CommandExecuted { id: script.id.clone(), success: false });
                        ipc::send(IpcEvent::Error { message: e });
                    }
                }
            } else {
                error!("Script '{}' not found on disk (deleted?), ignoring intent", cmd_config.id);
                voices::play_not_found();
                ipc::send(IpcEvent::Error {
                    message: format!("Script '{}' was deleted — restart Jarvis to update voice triggers", cmd_config.id),
                });
                ipc::send(IpcEvent::Idle);
            }
        } else {
        ...
        info!("[ROUTING] Script matched (live disk read): '{}' -> id='{}'", text, script.id);
        if !script.response_sound.is_empty() {
            audio::play_sound(&SOUND_DIR.join(&script.response_sound));
        } else {
            voices::play_random_from(script.get_sounds(&i18n::get_language()).as_slice());
