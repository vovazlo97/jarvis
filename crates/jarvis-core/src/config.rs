...
// Raised from 0.75: at 0.75 the classifier fires on phonetically similar but
// semantically different phrases (e.g. "гитхаб" → open_youtube at 0.82).
...
/// Separate (stricter) threshold for script fuzzy matching.
/// Scripts fire on the "last resort" fallback path, so we require higher
/// confidence to avoid false positives when no regular command matches.
pub const SCRIPT_RATIO_THRESHOLD: f64 = 95f64;
