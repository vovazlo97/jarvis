...
pub struct JCommand {
    ...
    // Regex patterns for matching (takes priority over fuzzy phrase matching)
    #[serde(default)]
    ...
    // Custom response sound (relative path from SOUND_DIR, e.g. "voices/pack/ru/ok1.wav")
...
impl JCommand {
    ...
    pub fn new_script_ref(
        id: String,
        phrases: std::collections::HashMap<String, Vec<String>>,
        sounds: std::collections::HashMap<String, Vec<String>>,
    ) -> Self {
        Self {
            id,
            cmd_type: "script_ref".to_string(),
